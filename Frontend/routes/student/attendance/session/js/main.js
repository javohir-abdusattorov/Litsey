...
$.ajax({
  type: 'GET',
  url: `${mainUrl}/api/v1/attendance/student-session`,
  headers: { "Authorization": accessToken },

  success(res) {
    console.log(res.data)
    $(".average-attendance").text(`${res.data.average}%`)
    const elAttendanceContainer = $(".attendace-container").html(``)
    for (const day of res.data.attendances) createDayAttendance(day.data, elAttendanceContainer, day.weekDay)
